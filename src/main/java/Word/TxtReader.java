...
public class TxtReader {
    ...
    private static int threadNum = 4;//初始化线程数
    ...
    public TxtReader(String filePath,int bufSize,int threadNum,CountDownLatch countDownLatch){
     ...
     * 启动多线程读取文件
     */
    public void startRead(NLPIR nlpir){
            ...
            try {
                                ...
                                if(startIndex != 0){//如果不是第一个数据段
                                    if(lineCount == 1){
                                        if(isWholeLine){//当且仅当第一行为完整行时才处理
                                            dataProcessHandler.process(line,nlpir);
                                        }
                                    }else{
                                        dataProcessHandler.process(line,nlpir);
                                    }
                ...
                if(temp.length > 0){
                    if(dataProcessHandler != null){
                        dataProcessHandler.process(temp,nlpir);
                    }
                }
            } catch (IOException e) {
                e.printStackTrace();
